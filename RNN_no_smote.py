...
class RNNModel(nn.Module):
    def __init__(self, input_size, hidden_size=64, num_layers=2, num_classes=2, dropout_rate=0.3):
        ...
        # RNN层
        #序列长度为1(一个时间步)，因为是非时序数据
        self.rnn = nn.LSTM(
            input_size=input_size, 
            hidden_size=hidden_size,
            num_layers=num_layers,
            #由于LSTM要求输入是3D，需要修改输入格式
            batch_first=True, #设置输入格式为：(batch_size, seq_len, input_size)即（32，1，13）
...
def main():
    ...
    # 创建模型
    input_size = X_train.shape[1]  # 确保input_size为特征数
    model = RNNModel(
        input_size=input_size,
        hidden_size=64,
        num_layers=2,
    ...
    plt.subplot(122)
    sns.heatmap(confusion_matrix(y_test, test_predicted),  # 修改为使用test_predicted
                annot=True, fmt='d', cmap='Blues')
    plt.title('测试集混淆矩阵')
    plt.xlabel('预测标签')
    plt.ylabel('真实标签')
